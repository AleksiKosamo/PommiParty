// ── Shared types (canonical — mirrored in client/src/types.ts) ───────────
...
export interface Player {
  ...
  isHost: boolean;
  lives: number;
  wordsAnswered: number; // words this player personally answered correctly
  usedLetters: string[];
}
...
export interface GameState {
  ...
  round: number;
  maxRounds: number;
  wordCount: number; // total correct words answered in this session (for difficulty scaling)
  isRanked: boolean; // whether this is a ranked game
}
